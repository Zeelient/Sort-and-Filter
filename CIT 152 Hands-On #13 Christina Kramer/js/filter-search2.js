(function() {                             // Lives in an IIFE
  ...
  var $image = $('#filter-image-search');      // Get the input element
  ...
  if ('oninput' in $image[0]) {          // If browser supports input event
    $image.on('input', filter);          // Use input event to call filter()
  } else {                                // Otherwise
    $image.on('keyup', filter);          // Use keyup event to call filter()
  }              
